...
from rpython.rlib.rarithmetic import intmask, r_uint32, r_uint, string_to_int, r_int32
...
def entry_point(argv):
    ...
    qry, kmer, Ns = args['-i'], int(args['-k']), int(args['-n'])
    kmer = min(max(1, kmer), 31)
    size = int(pow(4, kmer)+1)
    print('size', size)
    kmer_dict = [r_int32(1)] * size
    for i in xrange(size):
        kmer_dict[i] = r_int32(i)
    #kmer_dict[2] = 2
    #print(kmer_dict[0:10])
    #kmer_dict = {}
    #kmer_dict = [[] for elem in xrange(int(pow(4, kmer)+1))]
    #print('kmer', kmer, len(kmer_dict))
    N = 0
    #mx = 1<<62
    #mx = 0b100
    #print(mx)
    #ht = [[mx]  for elem in xrange(Ns)]
    #for i in xrange(Ns):
    #    kmer_dict[i] = mx
    #print(ht[0])
    #raise SystemExit()
    ...
    f = open(qry, 'r')
    for hd, seq in parse(f):
        n = len(seq)
        #for i in xrange(n- kmer + 1):
        #    j = max(i + kmer, 0) 
        #    subseq = seq[i: j]
        #    k = k2n_(subseq)
        for k in seq2ns_(seq, kmer):
            kmer_dict[k] = r_int32(k)
            #pass
            #try:
            #    kmer_dict[k].append(k)
            #except:
            #    kmer_dict[k] = [k]
